...
1. 赛事状态 `open` 时可报名
2. 校验：信誉积分 ≥ 80、NTRP 在范围内、性别符合、未被创建者拉黑、人数未满
...
按参赛者 NTRP 降序排列（使用现有 `_ntrp_to_float()`），NTRP 相同按信誉积分降序，仍相同则随机。排好后写入 `EventParticipant.seed`。
...
- 缺席方触发信誉积分扣罚（复用 `apply_credit_change()`，走约球同样的扣分规则）
...
booking、review、credit、matching 等现有模块完全不动。信誉积分扣罚复用 `apply_credit_change()`。
