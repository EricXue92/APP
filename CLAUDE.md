...
- **Notification system**: `services/notification.py` + `routers/notifications.py` — in-app notifications created by direct service calls at trigger points. Covers booking events (joined/accepted/rejected/cancelled/confirmed/completed), follow/mutual, review blind reveal, and admin report resolution (resolved/warned/suspended). No push delivery; iOS client polls REST API. `NotificationType` enum defines all 12 event types. `create_notification()` is internal-only (not exposed via router).
